    ...
    public function handle()
    {
       // zoho script 
        ...
        $json = array (
            ...
            from Leads
            where Modified_Time between '".Carbon::createFromDate(2020, 7, 20)->startOfDay()->toIso8601String()."' and  '". Carbon::createFromDate(2020, 7, 27)->endOfDay()->toIso8601String() ."'"
        );
        /*
        $json = array (
        "select_query" => "select Full_Name, Email, Phone, Description, Estado, Marca, Producto, Fecha_de_visita_al_Showroom, Hora_de_visita_al_showroom, Fecha_de_cooking_demo, Fecha_de_la_llamada, Hora_de_la_llamada, UTM_Anuncio_ID, UTM_Campaign_Name, UTM_Source, Lead_Source, Created_Time, Modified_Time, SalesManago_Contact_ID
        ...
        if(isset($contactsZoho) && sizeof($contactsZoho->data) > 0 ){
            ...
            if(sizeof($contactosConId)>0){
                foreach($contactosConId as $contactZoho){
                    ...
                    $salesContacto = end($salesContactos->contacts);
                    unset($salesContactos);
                    if($salesContacto) // si existe el user en sales manago
                    {
                        ...
                        //*/
                        //array_push($listModifiedSales, $salesContacto);
                    }
                }
            }
            ...
            if(sizeof($contactosSinId)>0){
                foreach($contactosSinId as $contactZoho){
                    ...
                    $salesContactos = $salesManago->getContactService()->listByEmails("sleal@iesa.cc",array( "email" => array($contactZoho->Email)));
                    $salesContacto = end($salesContactos->contacts);
                    unset($salesContactos);
                    if($salesContacto) // si existe el user en sales manago
                    {
                        ...
                        $record->setFieldValue("SalesManago_Contact_ID", $salesContacto->id);
                        array_push($moficacionesFZOHO, $record);
